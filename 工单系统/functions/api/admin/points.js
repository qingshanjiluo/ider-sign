...
export async function onRequest(context) {
  ...
  try {
    ...
    // 记录操作日志到 order_activities（用 NULL order_id 表示系统操作，避免 FK 约束）
    await env.DB.prepare(
      "INSERT INTO order_activities (order_id, user_id, action, detail) VALUES (?, ?, ?, ?)"
    ).bind(
      null,
      user.id,
      'admin_grant_points',
    ...
    console.error('points POST error:', e);
    return json({ error: '发放积分失败: ' + (e.message || '数据库错误') }, 500);
  }
}
